    ...
    <div class="row justify-content-center">
        <div class="col-md-6 col-sm-10 auth-form-wrapper">
            ...
            <form method="POST" action="{{ route('public.member.login') }}">
              @csrf
              <div class="mb-3">
                  <input id="email" type="email" class="form-control{{ $errors->has('email') ? ' is-invalid' : '' }}" placeholder="{{ trans('plugins/member::dashboard.email') }}" name="email" value="{{ BaseHelper::stringify(old('email')) }}" autofocus>
                  @if ($errors->has('email'))
